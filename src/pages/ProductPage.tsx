...
export default function ProductPage() {
  ...
  if (loading) {
    return <div className="artwork-page__status">{t('product.page.loading')}</div>
  }

  if (error || !artwork) {
    return (
      <div className="artwork-page__status">
        <p>{t('product.page.notFound')}</p>
        <Link to="/" className="artwork-page__back">{t('product.page.backToGalleryFull')}</Link>
      </div>
    )
  }
    ...
    <>
      <div className="artwork-page">
        <nav className="artwork-page__topbar">
          <Link to="/" className="artwork-page__back">{t('product.page.backToGallery')}</Link>
        </nav>
        ...
        <div className="artwork-page__layout">
          <div className="artwork-page__img-wrap">
            ...
            <button
              ...
              onClick={() => setFullscreen(true)}
              aria-label={t('product.page.viewFullscreen')}
            >
          ...
          <div className="artwork-page__info">
            ...
            {artwork.externalLink && (
              ...
              >
                ...
                {t('product.page.viewOnInstagram')}
              </a>
            )}

            <ShareLinks
              url={`${siteConfig.baseUrl}/${siteConfig.productPath}/${artwork.slug}`}
            ...
            <div className="artwork-page__purchase">
              {artwork.sold && (
                <span className="artwork-page__sold">{t('product.page.sold')}</span>
              )}
            ...
            <nav className="artwork-page__nav">
              {prev
                ? <Link to={`/${siteConfig.productPath}/${prev.slug}`} className="artwork-page__nav-btn">‹ {prev.title}</Link>
                : <span />
              }
              {next
                ? <Link to={`/${siteConfig.productPath}/${next.slug}`} className="artwork-page__nav-btn artwork-page__nav-btn--next">{next.title} ›</Link>
                : <span />
              }
            </nav>
          </div>
      ...
      {fullscreen && (
        ...
        >
          ...
          <button
            ...
            aria-label={t('product.page.closeFullscreen')}
          >
            ✕
          </button>
        </dialog>
